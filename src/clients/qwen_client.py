...
class QwenClient:
    ...
    def with_structured_output(self, model_class: type):
        return LangChainStructuredWrapper(self, model_class)


class LangChainStructuredWrapper:
    """Adapter that provides a LangChain-like `with_structured_output(...).invoke(messages)` API.

    It builds a function-style tool schema from a Pydantic model's JSON schema
    and uses `qwen_client.call_qwen_structured` under the hood, returning a
    validated Pydantic model instance.
    """

    def __init__(self, client: QwenClient, model_class: type):
        self._client = client
        self._model = model_class

        # Obtain JSON schema from Pydantic v2 or v1
        schema = None
        try:
            # Pydantic v2
        ...
        self._tool_schema = {
            "function": {
                "name": getattr(model_class, "__name__", "structured_function"),
                "parameters": schema,
            }
        }

    def invoke(self, messages: Sequence[Mapping[str, str]]):
        raw = self._client.call_qwen_structured(messages, self._tool_schema)
        if raw is None:
            return None

        # Convert raw dict into Pydantic model instance
        try:
            # Prefer Pydantic v2 parsing
            if hasattr(self._model, "model_validate"):
                return self._model.model_validate(raw)
            if hasattr(self._model, "parse_obj"):
                return self._model.parse_obj(raw)
            return self._model(**raw)
        except Exception:
            # Fallback: return raw dict if model instantiation fails
            return raw
